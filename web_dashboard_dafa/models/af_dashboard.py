    ...
    def create(self, vals):
        ...
        if res:
            for user in res.group_id.users:
                ...
                if dashboard:
                    cleanr = re.compile('<.*?>')
                    same_box = dashboard.box_ids.filtered(lambda b: re.sub(cleanr, '', b.description) == re.sub(cleanr, '', res.description))
                    if same_box:
                ...
                if box.description and box.user_id and not box.hide:
                    ...
                    if same_boxes:
                        for same_box in same_boxes:
                            if re.sub(cleanr, '', box.description) == re.sub(cleanr, '', same_box.description):
                                same_box.hide = True
                    ...
                    for g_box in group.box_ids:
                        if g_box.description:
                            ...
                            same_box = dashboard.box_ids.filtered(
                                lambda b: re.sub(cleanr, '', b.description) == re.sub(cleanr, '', g_box.description))
                            if same_box:
                                dashboard.box_ids = [(0, 0, {
                                    'af_dashboard_id': dashboard.id,
                                    'name': g_box.name,
